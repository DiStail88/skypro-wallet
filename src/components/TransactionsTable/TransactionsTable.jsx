import {
  TransactionsContainer,
  Table,
  TableHeading,
  TableThead,
  TableTr,
  Tableth1,
  TableTd,
  TableDeleteButton,
} from "./TransactionsTable.styled.js";
import { deleteTransaction } from "../../services/api"; // импорт функции удаления

// Перевод категорий
const CATEGORY_TRANSLATIONS = {
  food: "Еда",
  transport: "Транспорт",
  housing: "Жильё",
  joy: "Развлечения",
  education: "Образование",
  others: "Другое",
};

export default function TransactionsTable({ transactions = [], onDelete }) {
  const handleDelete = async (id) => {
    if (onDelete) {
      try {
        await deleteTransaction(id); // удаляем через API
        onDelete(id); // обновляем список в родителе
      } catch (error) {
        alert("Ошибка при удалении транзакции: " + error.message);
      }
    } else {
      alert(`Удалить транзакцию с id: ${id}`);
    }
  };

  return (
    <TransactionsContainer>
      <TableHeading>Таблица расходов</TableHeading>
      <Table>
        <TableThead>
          <TableTr>
            <Tableth1>Описание</Tableth1>
            <Tableth1>Категория</Tableth1>
            <Tableth1>Дата</Tableth1>
            <Tableth1>Сумма</Tableth1>
            <Tableth1></Tableth1>
          </TableTr>
        </TableThead>
        <tbody>
          {transactions.length === 0 ? (
            <TableTr>
              <TableTd
                colSpan="5"
                style={{ textAlign: "center", padding: "1rem" }}
              >
                Нет транзакций
              </TableTd>
            </TableTr>
          ) : (
            transactions.map((tx) => (
              <TableTr key={tx._id}>
                <TableTd>{tx.description}</TableTd>
                <TableTd>
                  {CATEGORY_TRANSLATIONS[tx.category] || tx.category}
                </TableTd>
                <TableTd>{new Date(tx.date).toLocaleDateString()}</TableTd>
                <TableTd>{tx.sum ?? tx.amount} ₽</TableTd>
                <TableTd>
                  <TableDeleteButton onClick={() => handleDelete(tx._id)}>
                    <svg
                      width="12"
                      height="12"
                      viewBox="0 0 12 12"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.62 2.79L9.41 2.79L7.72 1.1C7.59 0.96 7.37 0.96 7.23 1.1C7.09 1.23 7.09 1.45 7.23 1.59L8.42 2.79L3.57 2.79L4.76 1.59C4.9 1.45 4.9 1.23 4.76 1.1C4.63 0.96 4.4 0.96 4.27 1.1L2.58 2.79L2.38 2.79C1.93 2.79 1 2.79 1 4.07C1 4.55 1.09 4.87 1.31 5.08C1.42 5.2 1.57 5.27 1.72 5.31C1.87 5.34 2.03 5.35 2.17 5.35L9.82 5.35C9.97 5.35 10.12 5.34 10.26 5.31C10.68 5.2 11 4.9 11 4.07C11 2.79 10.06 2.79 9.62 2.79Z"
                        fill="#999999"
                      />
                      <path
                        d="M9.52 6L2.43 6C2.12 6 1.89 6.27 1.94 6.58L2.36 9.14C2.5 10.01 2.87 11 4.54 11L7.34 11C9.03 11 9.33 10.15 9.51 9.2L10.01 6.59C10.07 6.28 9.84 6 9.52 6ZM5.3 9.22C5.3 9.41 5.15 9.57 4.96 9.57C4.76 9.57 4.61 9.41 4.61 9.22L4.61 7.57C4.61 7.38 4.76 7.22 4.96 7.22C5.15 7.22 5.3 7.38 5.3 7.57L5.3 9.22ZM7.44 9.22C7.44 9.41 7.29 9.57 7.09 9.57C6.9 9.57 6.74 9.41 6.74 9.22L6.74 7.57C6.74 7.38 6.9 7.22 7.09 7.22C7.29 7.22 7.44 7.38 7.44 7.57L7.44 9.22Z"
                        fill="#999999"
                      />
                    </svg>
                  </TableDeleteButton>
                </TableTd>
              </TableTr>
            ))
          )}
        </tbody>
      </Table>
    </TransactionsContainer>
  );
}
